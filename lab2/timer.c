...
{
    if(freq > TIMER_MAX_FREQ) { //div would be equal to 0, due to it being stored in an integer
      printf("Frequency is too high\n");
      return 1;
    }

    if(freq < TIMER_MIN_FREQ) { //div would be bigger than the maximum capacity for an uint_8 (overflow)
...
int (timer_subscribe_int)(uint8_t *bit_no)
{
    *bit_no = BIT(hook_id); //sets the bitmask for irq_set
    ...
    if(sys_irqsetpolicy(TIMER0_IRQ, IRQ_REENABLE, &hook_id) != OK) { //function returns OK on success
        return 1;
    }
...
{
    if(sys_irqrmpolicy(&hook_id) != OK) { //function returns OK on success 
      return 1;
    }
    else return 0;
}
